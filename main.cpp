...
#if defined(NDEBUG)
# error "Sanddollar cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Sanddollar Signed Message:\n";
...
CAmount GetBlockValue(int nHeight, const CAmount& nFees)
{
    ...
    {//876 block per day
        int nPerMonth = 26666;
        if(nHeight<nPerMonth*3)		   //first half year 8mili coin
            nSubsidy = 12.5*COIN;
        else if(nHeight<nPerMonth*6)    //second half year 10mili coin
            nSubsidy = 62.5*COIN;
        else if(nHeight<nPerMonth*12)    //second year 30mili coin
            nSubsidy = 62.5*COIN;
        else if(nHeight<nPerMonth*24)    //3、、、year 73mili coin
            nSubsidy = 93.75*COIN;
        else if(nHeight<nPerMonth*36)    //3 year 50mili coin
            nSubsidy = 156.25*COIN;
        else if(nHeight<nPerMonth*48)    //8 year 30mili coin
            nSubsidy = 312.5*COIN;
        else if(nHeight<nPerMonth*72)    //6 year 15mili coin
            nSubsidy = 156.25*COIN;
        else if(nHeight<nPerMonth*108)    //10 year 15mili coin
            nSubsidy = 125*COIN;
        else if(nHeight<nPerMonth*120)    //10 year 15mili coin
            nSubsidy = 106.25*COIN;
		else		// 10 year complete no money pay
			nSubsidy = 0*COIN;
	}
    return nSubsidy + nFees;
}
...
bool IsInitialBlockDownload()
{
	...
	LogPrintf("IsInitialBlockDownload,pindexBestHeader->nHeight:%d,datetime:%d\n",pindexBestHeader->nHeight,
		pindexBestHeader->GetBlockTime() < GetTime() - 24 * 60 * 60);

    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 * 5 ||
            pindexBestHeader->GetBlockTime() < GetTime() - 24 * 60 * 60 * 10);
    if (!state)
        lockIBDState = true;

	LogPrintf("IsInitialBlockDownload return: %d\n",state);
    return state;
}
...
void ThreadScriptCheck() {
    RenameThread("sanddollar-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Sanddollar: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Sanddollar: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
